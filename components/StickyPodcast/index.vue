  ...
  >
    ...
    >
      ...
      <div
        ...
        :class="{
          'sticky-podcast__player-mini-controls--playing':
            podPlaying || shouldPlay,
...
export default {
  ...
  computed: {
    ...
    player() {
      return this.activePlayer
        ? this.$store.getters["content/player"](this.activePlayer)
        : null;
    },
    pods() {
  ...
  watch: {
    currentTime() {
      ...
      if (this.currentPod.start) {
        this.progress =
          ((this.currentTime - this.currentPod.start) /
            (this.currentPod.end - this.currentPod.start)) *
          100;
    ...
    currentPod(newVal, oldVal) {
      ...
      if (
        this.currentPod &&
        this.activePodcast &&
        this.currentPod.id !== this.activePodcast.id
      ) {
  ...
  methods: {
    ...
    watchTime() {
      this.currentTime = this.$refs.audioPlayer
    ...
    progressClick(e) {
      ...
      this.$refs.audioPlayer.currentTime = this.currentPod.start
        ? this.currentPod.start + this.duration * percentage
        : this.duration * percentage;
      this.setPlaying(true);
    },
    handleScroll() {
      if (
        !this.activePodcast ||
        !document.querySelector(".main-section__inner") ||
        !document.querySelector(".main-section__inner").offsetParent
      )
        return;
      if (
        window.scrollY >
        document.querySelector(".main-section__inner").offsetParent.offsetTop
      ) {
    ...
    handlePlayPause() {
      ...
      if (this.podPlaying) {
        if (this.readyToPlay) {
          this.$gtag &&
            this.$gtag.event(`${this.currentPod.type === "clip"
                  ? "poddcast-clip"
                  : "podcast-episode"}_played`, {
              event_category:
                this.currentPod.type === "clip"
                  ? "poddcast-clip"
                  : "podcast-episode",
              event_action: "played",
