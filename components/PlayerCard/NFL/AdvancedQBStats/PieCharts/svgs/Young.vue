<template>
  <svg viewBox="0 0 320.16 253.67">
    <path
      style="opacity:0.6;fill-rule:evenodd;clip-rule:evenodd;fill:#176956;"
      d="M126.52,127.01L161.76,5.58c47.42,13.76,80.93,51.22,89.35,99.87L126.52,127.01z"
    />
    <path
      style="opacity:0.7;fill-rule:evenodd;clip-rule:evenodd;fill:#176956;"
      d="M126.52,127.01L65.57,16.23C96.04-0.53,128.36-4.11,161.76,5.58L126.52,127.01z"
    />
    <path
      style="opacity:0.8;fill-rule:evenodd;clip-rule:evenodd;fill:#176956;"
      d="M126.52,127.01L7.84,83.39c11.03-30.02,29.71-51.74,57.73-67.16L126.52,127.01z"
    />
    <path
      style="opacity:0.9;fill-rule:evenodd;clip-rule:evenodd;fill:#176956;"
      d="M126.52,127.01L53.67,230.36C6.07,196.8-12.25,138.05,7.84,83.39L126.52,127.01z"
    />
    <path
      style="fill-rule:evenodd;clip-rule:evenodd;fill:#176956;"
      d="M126.52,127.01l124.59-21.56c11.91,68.81-34.22,134.24-103.03,146.15c-34.47,5.97-65.82-1.09-94.41-21.25
					L126.52,127.01z"
    />
	
    <text transform="matrix(1 0 0 1 122.1771 177.5391)"><tspan
      x="0"
      y="0"
      style="fill:#E4E4E4;fontSize: 12px"
    >Stand &amp; Deliver</tspan><tspan
      x="26.11"
      y="14.4"
      style="fill:#E4E4E4;fontSize: 12px"
    >36.7%</tspan></text>
    <text transform="matrix(1 0 0 1 41.9762 57.7199)"><tspan
      x="0"
      y="0"
      style="fill:#E4E4E4;fontSize: 12px"
    >Take a</tspan><tspan
      x="4.49"
      y="14.4"
      style="fill:#E4E4E4;fontSize: 12px"
    >Sack</tspan><tspan
      x="3.91"
      y="28.8"
      style="fill:#E4E4E4;fontSize: 12px"
    >11.2%</tspan></text>
    <text transform="matrix(1 0 0 1 32.6983 136.7493)"><tspan
      x="0"
      y="0"
      style="fill:#E4E4E4;fontSize: 12px"
    >Escape</tspan><tspan
      x="-3.97"
      y="14.4"
      style="fill:#E4E4E4;fontSize: 12px"
    >Pressure</tspan><tspan
      x="3.33"
      y="28.8"
      style="fill:#E4E4E4;fontSize: 12px"
    >20.4%</tspan></text>
    <text transform="matrix(1 0 0 1 91.0364 34.4166)"><tspan
      x="0"
      y="0"
      style="fill:#FFFFFF;fontSize: 12px"
    >Scramble</tspan><tspan
      x="10.78"
      y="14.4"
      style="fill:#FFFFFF;fontSize: 12px"
    >12.2%</tspan></text>
    <text transform="matrix(1 0 0 1 156.7437 73.543)"><tspan
      x="0"
      y="0"
      style="fill:#FFFFFF;fontSize: 12px"
    >Unchartable</tspan><tspan
      x="18.73"
      y="14.4"
      style="fill:#FFFFFF;fontSize: 12px"
    >17.3%</tspan></text>
  </svg>
</template>
<script>
export default {
  name: "YoungPie"
}
</script>

<style lang="scss" scoped>
  text tspan{
    @include advanced-bar-label;
    font-weight: normal; 
  }
</style>